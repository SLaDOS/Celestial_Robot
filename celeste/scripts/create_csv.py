...
MAX_INT = 11000.0
# Edinburgh
EDIN_LAT = 55.945011324580385
EDIN_LON = -3.1867749119995956
POL_ANGLES = {
...
TEST_PATH = '../my_bags/saved/'

test_names = glob.glob(TEST_PATH+'test_2024*/')
for test_name in test_names:
    print(test_name)
    bagfiles = glob.glob(test_name+"pol_op*")
    ...
    for i_b, bagname in enumerate(bagfiles):
        ...
        sun = ephemeris.Sun(observer.Observer(EDIN_LON, EDIN_LAT, date=record_time, degrees=True))
        ...
        for _data_id in range(len(pols[0])):
            ...
            for _pol_id in range(len(pols)):
                ...
                z += c * np.exp(1j * POL_ANGLES['pol_op_' + str(_pol_id)])

                new_pol_row = {'time': record_time, 'unit_type': 'POL', 'response': p,
                               'sun_azimuth': np.degrees(sun.az), 'sun_elevation': np.degrees(sun.alt),
                               'yaw': np.degrees(yaw[_data_id]-start_yaw),
                               'device_yaw': np.degrees(POL_ANGLES['pol_op_' + str(_pol_id)]),
                               'rotation': i_b+1,
                               'session': 'session1'
                               }
    ...
    df.to_csv(f'./csv/{test_name[-22:-1]}.csv', index=False)
